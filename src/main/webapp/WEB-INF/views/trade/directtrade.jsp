

<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<html>
<head>
    <title>title</title>
</head>
<!-- bootstrap css -->
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css"
      integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">

<!-- bootstrap js -->
<script src="https://code.jquery.com/jquery-3.4.1.slim.min.js"
        integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n"
        crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js"
        integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo"
        crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"
        integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6"
        crossorigin="anonymous"></script>
<link href="../../css/directtrade.css" rel="stylesheet">
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
    function newAddr() {
        $('.tradeName').val("");
        $('.tradePhone').val("");
        $('.tradeAddrInput').val("");
    };

    function addr() {
        $('.tradeName').val("김수호");
        $('#tradePhone_1').val("010");
        $('#tradePhone_2').val("1234");
        $('#tradePhone_3').val("5678");

        $('#tradeNewAddr1').val("서울특별시 강남구 압구정동 293길 20 OO아파트");
        $('#tradeNewAddr2').val("101동 101호")
    }
</script>
<body>
<div class="container">
    <div class="row">
        <div class="col-12">
            <label id="payMentLabel">결제하기</label>
        </div>
    </div>
    <form action="" method="post">
        <div class="orderContainsDiv row">
            <div class="col-9">
                <div id="marginDiv">
                    <label id="orderLabel">주문 내역</label><br>
                    <img id="orderImg"
                         src="data:image/jpeg;base64,">
                    <label id="titleLabel"><a id="titleA" href="#">제목(ex:스토리가 필요한 모든 영상컨텐츠의 시놉시스와 대본을 써 드립니다.)</a>
                    </label>
                    <label id="nameLabel">이름(ex:김완주)</label></div>
                <br>
                <label id="priceLabel">가격 : (30,000)Point</label><br><br>
                <div class="myContentsDiv">
                    <label id="myImformLabel">내 정보</label>
                    <br><br>
                    <div class="labelDiv">
                        <label id="addrLabel">배송지 선택
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="1" id="trade_Addr"
                                                                 name="addr_radio" checked="checked" onclick="addr();">
                            기본
                            배송지
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="2" id="trade_New_Addr"
                                                                 name="addr_radio" onclick="newAddr();"> 신규 배송지
                        </label>
                        <table id="myInformTable">
                            <colgroup>
                                <col width="120px">
                            </colgroup>
                            <tr>
                                <td class="nameClass">이름&nbsp;&nbsp;&nbsp;</td>
                                <td><input type="text" class="tradeName" name="tradeName" value="김수호"></td>
                            </tr>
                            <tr>
                                <td class="nameClass">전화번호&nbsp;&nbsp;&nbsp;</td>
                                <td>
                                    <input type="text" class="tradePhone" id="tradePhone_1" name="tradePhone_1" value="010">&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;
                                    <input type="text" class="tradePhone" id="tradePhone_2" name="tradePhone_2" value="1234">&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;
                                    <input type="text" class="tradePhone" id="tradePhone_3" name="tradePhone_3" value="5678">
                                </td>
                            </tr>
                            <tr>
                                <td class="nameClass">주소&nbsp;&nbsp;&nbsp;</td>
                                <td>
                                    <input type="text" class="tradeAddrInput" id="tradeNewAddr1" name="tradeNewAddr1" value="압구정동">
                                    <input type="button" value="우편 찾기">
                                </td>
                                <td rowspan="2"></td>
                            </tr>
                            <tr style="height: 0;">
                                <td class="nameClass"></td>
                                <td><input type="text" class="tradeAddrInput" id="tradeNewAddr2" name="tradeNewAddr2"
                                           placeholder="상세 주소 입력" value="상세 주소"></td>
                            </tr>
                        </table>
                    </div>
                </div>
                <hr>
                <div id="directMapDiv">
                    이곳엔 지도가 들어갑니다.
                </div>
            </div>
            <div class="col-3">
                <div class="labelDiv">
                    <table id="orderTable">
                        <colgroup>
                            <col width="130px">
                            <col width="140px">
                        </colgroup>
                        <tr>
                            <td class="nameClass">보유 포인트</td>
                            <td>100,000 Point</td>
                        </tr>
                        <tr>
                            <td class="nameClass">가격</td>
                            <td>20,000 Point</td>
                        </tr>
                        <tr>
                            <td class="nameClass">결제 후 포인트</td>
                            <td>80,000 Point</td>
                        </tr>
                    </table>
                    <hr>
                </div>
                <br>
                <label id="orderCheckLabel">
                    <input type="checkbox" name="orderCheckBox">&nbsp;&nbsp;주문 확인(필수)</label><br><br>
                <input type="button" id="payBtn" value="결제하기" onclick="">
            </div>
        </div>
    </form>
</div>
</body>
</html>